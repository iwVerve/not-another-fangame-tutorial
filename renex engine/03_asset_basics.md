# Asset Basics
Assets are the various building blocks making up your game.
...
* Sounds are sounds. These are used by GameMaker's audio engine, which renex engine doesn't use - instead, sound files are stored in the `data` folder, next to the engine `source` folder. We'll go into the details of that later.
* Backgrounds are big images that can be used as, well, room backgrounds or tilesets.
* Objects are the things that make up your game - they exist somewhere in the current room, and they run their own logic. `Player`, `Block`, `Cherry`, `Warp`, etc. are all objects. The individual things that then exist in your game are called instances of objects. For example, a room may have many Blocks in it - these are different instances of one object. This is a small distinction, but it's worth noting.
...
You can try selecting some instances, dragging them around, or stretching some of the Blocks with the square handle at the bottom left of the selected instance.
...
Finally, let's change the background music played in this room. It is controlled by the `MusicPlayer` object, which you may find in the top-left corner of room. You can click on the bottom-left corner of the object to edit its fields. 
...
### Instances
The instances tab is where you place instances of objects in your room. Instances run nearly all logic in your game. They are selected in the object palette on the left, using the plus button at the end to add more to the palette from the project.
...
### Backgrounds
In the Backgrounds tab you can set up to 8 room backgrounds.You may also simply set a solid color as the basckground to the room. Backgrounds can be drawn tiled or stretched to fill the room, they can be static or moving, and they can be behind or in front of all the instances.
...
* Caption (optional): Sets the title of your game window. However, the engine overwrites the caption already, so there's no reason to change this.
...
**IMPORTANT**: When you export your game, you will have to copy the `data` folder together with the game exe. Otherwise you will see an error prompting you to do so :-)
